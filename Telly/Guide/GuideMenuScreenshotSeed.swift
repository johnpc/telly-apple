...
/// ``GuideGridModel/firstInfoTarget()``), so a plain `simctl` screenshot
...
enum GuideMenuScreenshotSeed {
    @MainActor
    static func present(_ target: Binding<GuideInfoTarget?>, model: GuideGridModel) async {
        ...
        guard let hit = model.firstInfoTarget() else { return }
